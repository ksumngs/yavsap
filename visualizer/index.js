...
function serveTraceDocument(prefix, suffix) {
  return function (req, res) {
    ...
    if (timestamp.toLowerCase() == "latest") {
      ...
      if (
        !getTraceDocuments(prefix, suffix)
        ...
        res.sendFile(
          path.join(__dirname + "/.trace/" + prefix + timestamp + suffix)
        );
      }
    }
  };
...
app.get("/phylogenetics/:sample", function (req, res) {
  ...
  newickData = fs
    .readFileSync(
      path.join(__dirname + "/phylogenetics/" + sampleName + ".nwk"),
...
app.get(
  "/nf-report/:timestamp",
  serveTraceDocument("execution_report_", ".html")
);
app.get(
  "/nf-timeline/:timestamp",
  serveTraceDocument("execution_timeline_", ".html")
);
...
app.use(
  "/assembly/alignment",
  express.static(__dirname + "/assembly/alignment")
);
...
app.use(
  "/js/phylotree",
  express.static(__dirname + "/node_modules/phylotree/dist")
);
...
app.listen(port, function () {
  console.log(
    "YAVSAP results visualizer running, available at http://localhost:" + port
  );
});
